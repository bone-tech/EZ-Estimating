...
public class MainWindow
{
	...
	public Stage getStage(Stage mainStage) 
	{
		...
		saveBttn.setOnAction(new EventHandler<ActionEvent>()
		{
			...
			public void handle(ActionEvent e)
			{
				Customer currentCustomer = new Customer(firstTextField.getText(), lastTextField.getText(), addressTextField.getText(), cityTextField.getText(), stateBox.getValue(), zipTextField.getText(), 
						countryTextField.getText(), homePhoneTextField.getText(), workPhoneTextField.getText(), faxTextField.getText());
				
				Vehicle currentVehicle = new Vehicle(vinTextField.getText(), vehicleYearTextField.getText(), vehicleMakeTextField.getText(), vehicleModelTextField.getText(), vehicleStyleBox.getValue(), 
						vehicleLicenseTextField.getText(), vehicleLicenseBox.getValue(), mileageInTextField.getText(), mileageOutTextField.getText(), finishTypeBox.getValue(), colorBox.getValue());
				
				Estimate currentEstimate = new Estimate(currentCustomer, currentVehicle, total.getText(), estimatorTextField.getText(), reportID, consoleGrid.getChildren().toString());
				
				Estimate.save(currentEstimate);
			/*String user = System.getProperty("user.home");
			String dirName = new String(user + File.separator + "Documents" + File.separator + "EZEstimating" +  File.separator + "Estimates");
			Path path3 = Paths.get(dirName);
			if(!Files.exists(path3)) {
